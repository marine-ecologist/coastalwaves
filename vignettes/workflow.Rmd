...
The `get_wind_data ()` function extracts daily wind data (speed, bearing) from the [NOAA Blended Daily Seawinds dataset](https://www.ncei.noaa.gov/products/blended-sea-winds).

To test functionality here, extract dates around November spawning window for the past 3 years and plot timeseries.

Wind data is extracted from the global dataset from the central grid cell overlapping the `whitsundays_union` object:

```{r, message=FALSE, warning=FALSE, eval=TRUE, cache=TRUE}



spawning_2022_wind <- get_wind_data(st_centroid(whitsundays_union), "2022-10-01", "2022-10-30")
spawning_2023_wind <- get_wind_data(st_centroid(whitsundays_union), "2023-10-01", "2023-10-30")
spawning_2024_wind <- get_wind_data(st_centroid(whitsundays_union), "2024-10-01", "2024-10-30")

# combine
whitsundays_weather <- rbind(spawning_2022_wind, spawning_2023_wind, spawning_2024_wind)
...
bearingplot <- whitsundays_weather |>  
  ...
  geom_line(aes(x = date, y = bearing, color=year), show.legend=FALSE) +
...
speedplot <- whitsundays_weather |>  
  ...
  facet_wrap(~year, scales="free", nrow=3) +
  geom_line(aes(x = date, y = windspeed_ms, color=year), show.legend=FALSE) +
...
whitsundays_waves <- calculate_wave_exposure(wind = whitsundays_weather, 
